...
export const typeChecker = (ast: IExpression[] = []): IError[] => {
    let errors: IError[] = [];
    ast.filter(node => node.type === NodeType.TYPE && !(node as any).imported).forEach(
        (node: IType) => {
            ...
            node.fields
                ...
                .forEach((field: ITypeField) => {
                    ...
                    if (!ref) {
                        errors.push({
                            message: `Cannot find type "${typeId}" of field "${
                                field.id
                            }" of type "${node.id}"`,
                    ...
                    for (let i = 0; i < field.ofType_params.length; ++i) {
                        ...
                        if (!paramRef) {
                            errors.push({
                                message: `Cannot find type "${paramId}" of field "${
                                    field.id
                                }" of type "${node.id}"`,
            ...
            node.fields
                ...
                .forEach(
                    (field: IPluckedField): void => {
                        ...
                        if (!ref) {
                            errors.push({
                                message: `Cannot find type "${typeId}" to pluck from on type ${
                                    node.id
                                }`,
                                ...field.parts_start[0]
                            });
                        ...
                        let refField = (ref as IType).fields.find(
                            f =>
                                ...
                                (f as ITypeField).id === field.parts[1]
                        );
                        if (!refField) {
                            errors.push({
                                message: `Cannot find field "${
                                    field.parts[1]
                                }" of type "${typeId}" to pluck`,
                                ...field.parts_start[1]
                            });
                        }
                    }
     ...
     */
    ast.filter(node => node.type === NodeType.AGGREGATE).forEach(
        (aggregate: IAggregate): void => {
            ...
            aggregate.valueObjects.forEach((v: string, i: number) => {
                ...
                if (!valueObject) {
                    errors.push({
                        message: `Cannot find the Value Object "${v}" on Aggregate "${
                            aggregate.root
                        }"`,
     ...
     */
    ast.filter(node => node.type === NodeType.VIEW).forEach(
        (view: IView): void => {
            view.nodes.forEach((v: string, i: number) => {
                let valueObject = getNodeById(ast, [], v);
                if (!valueObject) {
                    errors.push({
                        message: `Cannot find the Node "${v}" on View "${view.id ||
                            "Unnamed view"}"`,
                        ...view.nodes_start[i]
                    });
                }
